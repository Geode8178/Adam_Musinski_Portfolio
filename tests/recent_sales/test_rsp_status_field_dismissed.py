...
def test_rsp_status_field_dismissed(page):
    def _log(msg: str) -> None:
        print(f"[RSP][Status=Dismissed] {msg}")
    ...
    # Status: set to "Dismissed" (#id_status)
    _log('Setting Status = "Dismissed".')
    ...
    status_dropdown.select_option(label="Dismissed")

    selected_status = status_dropdown.locator("option:checked").inner_text().strip()
    assert selected_status == "Dismissed", (
        f"Expected Status selection to be 'Dismissed', got {selected_status!r}"
    )
    ...
    if not has_rows:
        try:
            ...
            pytest.skip(
                "No records found for Market Actor='AF Supply' and Status='Dismissed' (Total Record Count 0)."
            )
        except Exception:
            raise Exception(
    ...
    def verify_status_cell(cell, *, stage: str) -> None:
        ...
        assert ("Dismissed" in cell_text) or ("Dismissed" in help_text_text), (
            "Expected 'Dismissed' to appear in the first-row Status cell "
        ...
        _log("Status cell OK: shows 'Dismissed' (text and/or help-text).")
    ...
    _log(
        "PASSED: Market Actor='AF Supply', Status='Dismissed', Invoice Date='All'; "
        "clicked Search; verified first-row Status cell indicates 'Dismissed'; "
