...
def test_rsp_status_field_exported(page):
    def _log(msg: str) -> None:
        print(f"[RSP][Status=Exported] {msg}")
    ...
    status_dropdown.select_option(label="Exported")

    selected_status = status_dropdown.locator("option:checked").inner_text().strip()
    assert selected_status == "Exported", (
        f"Expected Status selection to be 'Exported', got {selected_status!r}"
    )
    ...
    if not has_rows:
        try:
            ...
            pytest.skip(
                "No records found for Market Actor='AF Supply' and Status='Exported' (Total Record Count 0)."
            )
        except Exception:
            raise Exception(
    ...
    _log(
        "PASSED: Market Actor='AF Supply', Status='Exported', Invoice Date='All'; "
        "clicked Search; verified first-row Status cell indicates 'Exported'; "
