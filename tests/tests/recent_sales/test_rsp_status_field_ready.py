...
def test_rsp_status_field_ready(page):
    def _log(msg: str) -> None:
        print(f"[RSP][Status=Ready] {msg}")
    ...
    assert selected_status == "Ready", (
        f"Expected Status selection to be 'Ready', got {selected_status!r}"
    )
    ...
    if not has_rows:
        try:
            ...
            pytest.skip(
                "No records found for Market Actor='AF Supply' and Status='Ready' (Total Record Count 0)."
            )
        except Exception:
            raise Exception(
    ...
    _log(
        "PASSED: Market Actor='AF Supply', Status='Ready', Invoice Date='All'; "
        "clicked Search; verified first-row Status cell indicates 'Ready'; "
