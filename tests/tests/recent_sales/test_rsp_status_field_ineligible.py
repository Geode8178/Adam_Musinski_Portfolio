...
def test_rsp_status_field_ineligible(page):
    def _log(msg: str) -> None:
        print(f"[RSP][Status=Ineligible] {msg}")
    ...
    # Status: set to "Ineligible" (#id_status)
    _log('Setting Status = "Ineligible".')
    ...
    status_dropdown.select_option(label="Ineligible")

    selected_status = status_dropdown.locator("option:checked").inner_text().strip()
    assert selected_status == "Ineligible", (
        f"Expected Status selection to be 'Ineligible', got {selected_status!r}"
    )
    ...
    # Deterministic wait for results.
    ...
    def verify_status_cell(cell, *, stage: str) -> None:
        ...
        # Many apps render the status label inside span.help-text, but not always.
        help_text = cell.locator("span.help-text")
        help_text_text = help_text.first.inner_text().strip() if help_text.count() else ""

        assert ("Ineligible" in cell_text) or ("Ineligible" in help_text_text), (
            "Expected 'Ineligible' to appear in the first-row Status cell "
            f"(text={cell_text!r}, help_text={help_text_text!r})."
        )

        # Optional signal: if an icon is present, at least confirm it renders (don’t over-specify).
        ...
        _log("Status cell OK: shows 'Ineligible' (text and/or help-text).")
    ...
    _log(
        "PASSED: Market Actor='AF Supply', Status='Ineligible', Invoice Date='All'; "
        "clicked Search; verified first-row Status cell indicates 'Ineligible'; "
